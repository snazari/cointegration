function results = sdem_panel_FE(y,x,W,T,info)
% PURPOSE: ML SDEM model estimates for static spatial panels 
...
% ---------------------------------------------------
%  USAGE: results = sdem_panel_FE(y,x,W,T,info)
...
% ---------------------------------------------------
%  RETURNS: a structure
%         results.meth  = 'psdem' if infomodel=0
%                       = 'sdemsfe' if info.model=1
%                       = 'sdemtfe' if info.model=2
%                       = 'sdemstfe' if info.model=3
%         results.beta  = bhat
...
if model==0
    results.meth='psdem';
elseif model==1
    results.meth='sdemsfe';
elseif model==2
    results.meth='sdemtfe';
elseif model==3
    results.meth='sdemstfe';
else
    error('sdem_panel: wrong input number of info.model');
end
...
[N Ncol] = size(W);
if N ~= Ncol
error('sdem: wrong size weight matrix W');
elseif N ~= nobs/T
error('sdem: wrong size weight matrix W or matrix x');
end;
[nchk junk] = size(y);
if nchk ~= nobs
error('sdem: wrong size vector y or matrix x');
end;
...
if exitflag == maxit
fprintf(1,'\n sdem: convergence not obtained in %4d iterations \n',output.iterations);
end;
% return results 
...
% beta, beta
xpx(1:nvarw,1:nvarw) = (1/sige)*xs'*xs;
% rho, rho
xpx(nvarw+1,nvarw+1) = T*pterm;
% sige, sige
...
bvec = [results.beta
        results.rho];
results.tstat = bvec./(sqrt(tmp(1:nvarw+1,1)));
...
if hessn(nvarw+2,nvarw+2) == 0
 hessn(nvarw+2,nvarw+2) = 1/sige;  % this is a hack for very large models that 
...
zip = find(tmp <= 0);
 if length(zip) > 0
 tmp(zip,1) = 1;
 fprintf(1,'sdem: negative or zero variance from numerical hessian \n');
 fprintf(1,'sdem: replacing t-stat with 0 \n');
 end;
 bvec = [results.beta
        results.rho];
