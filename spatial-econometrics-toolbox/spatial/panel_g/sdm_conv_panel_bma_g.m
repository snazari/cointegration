function results = sdm_conv_panel_bma_g(y,x,Wmatrices,N,T,ndraw,nomit,prior)
% PURPOSE: Bayesian panel SDM model averaged estimates for all M combinations
...
% -------------------------------------------------------------
% USAGE: results = sdm_conv_gbma(y,x,Wmatrices,ndraw,nomit,prior)
...
%          a structure:
%          results.meth     = 'sdm_conv_panel_bma_g'
...
results.ntime = T;
if (nc ~= n)
       error('sdm_conv_panel_bma_g: wrong sized x-matrix');
end

if ndraw <= 1000
       error('sdm_conv_panel_bma_g: ndraw<=1000, increase ndraw to at least 10000');
end

if nargin == 7
            error('sdm_conv_panel_bma_g: you must add prior.model=0,1,2,3');
...
if thin == 1
    ...
    if m < 2
        error('sdm_conv_panel_bma_g: only one W-matrix');
    end
...
if eff < 999
    warning('sdm_conv_panel_bma_g: < 1000 draws after thining');
end


else
    error('sdm_conv_panel_bma_g: wrong # of input arguments to sdm_conv_panel_bma_g');
end
...
if parallel == 1
    ...
    parfor iter = 1:np
        ...
        res = sdm_conv_panel_g(y,x,Wmatrix(iter).model,N,T,ndraw,nomit,prior);
