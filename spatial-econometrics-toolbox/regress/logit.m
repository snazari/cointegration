...
while (iter < maxit) & (crit > tol)
    ...
    term1 = y.*(pdf./cdf); term2 = (iVec-y).*(pdf./(iVec-cdf));
    for kk=1:k;
        tmp1(:,kk) = term1.*x(:,kk);
        tmp2(:,kk) = term2.*x(:,kk);
    end;
    g = tmp1-tmp2; gs = (sum(g))';
    delta = exp(x*b)./(iVec+exp(x*b)); % see page 883 Green, 1997
...
for iCnt=1:t;
    xp = x(iCnt,:)';
    H = H - delta(iCnt,1)*(1-delta(iCnt,1))*(xp*x(iCnt,:));
end;
